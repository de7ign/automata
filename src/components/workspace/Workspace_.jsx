...
class Workspace_ extends React.Component {
    ...
    this.network.on("beforeDrawing", ctx => {
      ...
      if (this.finalStates !== null) {
        ...
        this.finalStates.forEach(value => {
          ...
          ctx.beginPath();
          ctx.arc(
            nodePosition[value].x,
            nodePosition[value].y,
            0,
            Math.PI * 2,
            36
          );
  ...
  handleNodeDialogKeyPress = event => {
    if (event.key === "Enter") this.handleNodeDialogEnterClose();
  };
  ...
  handleEdgeDialogKeyPress = event => {
    if (event.key === "Enter") this.handleEdgeDialogEnterClose();
  };
